...
### featureVersion 3（グループ・読みの特徴、2026-09-25）

v2 モデルの出力と入力 32ch にない死活の情報をプローブで探し（KataGo の入力にない候補 30 個から前進選択）、選んだ 4 つを選択に使っていない新しい対局 220 局で確かめた（死石判別の AUC +0.020、対局単位のブートストラップ 95% CI [+0.015, +0.026]。`docs/implementation-status.md` 2026-09-25）。v2 との違いは 4 チャンネルだけ。外した 4 つ（2 手前の石、空点、盤端）は他のチャンネルから作れるか、プローブで死石の情報がほぼなかったもの。4 つとも石の上だけに立ち、色を問わない（channel 0/1 と組み合わせて使う）、手番にもよらない。

| channel | v3 の意味 |
|---|---|
| 0〜3 | v2 と同じ（現局面、1手前の石） |
| 4 | その石のグループ（同じ色の連で、呼吸点を 2 つ以上共有するものをつなげた単位）の呼吸点が 8 以上 |
| 5 | その石のグループの眼の数（下記）が 2 以上 |
| 6〜15 | v2 と同じ |
| 16 | その石の連に、自分より呼吸点の多い相手の連が接している |
| 17〜26 | v2 と同じ |
| 27 | その石の連（呼吸点 3 以下）が、攻める側から打てば取られる（深さ 10・600 ノードまでの取りの読み。攻める側は連の呼吸点を空きの多い順に、守る側は呼吸点と、接する呼吸点 2 以下の相手の連の呼吸点を打つ。コウは無視、読み切れないときは取れないとする） |
| 28〜31 | v2 と同じ |

眼の数: 1 色だけに接する空点領域ごとに、1 点で欠け眼なら 0、7 点以上なら 2、2 点以下なら 1、それ以外は「そこに打つと領域が分かれる点」が 2 つ以上なら 2、1 つなら 1（急所を相手に打たれる側で数える）、0 なら 6 点で 2・それ以外 1。グループの呼吸点を含む領域（重複なし）について合計する。欠け眼の判定は v2 と同じ（斜めの相手の石が、辺・隅では 1 つ以上、中では 2 つ以上なら欠け眼）。計算は `Sources/IchiGoFeatures/GroupFeaturesV3.swift`（盤クラスを使わない配列で計算。1 局面あたり 9 路で約 70 µs、19 路で約 0.3 ms の追加、バッチ内は局面ごとに並列）。既存の局面行は `ichigo reencode --feature-version 3` で手順を再生して符号化し直せる（positionId は同じなので教師ラベルをそのまま結合できる）。
