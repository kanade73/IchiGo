...
### 2026-09-23 午後: LGN の高速化と、RinGo（b20c256）との組み合わせの測定

**探索の高速化。** プロファイルでは、既定設定（`--backend auto` = metal、leafBatch 8）の時間の大半が評価（GPU 待ちと CPU head）で、木の処理は約 15%。以下を追加した（既定の動作は変えず、フラグで選ぶ）:
- `--backend cpu-packed-mt [--cpu-workers N]`: バッチを 32 局面単位の塊に分けて全コアで `PackedCPUBackend` を並列実行（`Sources/LogicModel/ParallelPackedCPUBackend.swift`）。論理層は bit 一致、head は Accelerate の blocking 差で末尾 bit が揺れるため packed vs scalar と同じ許容誤差でテスト。
- `--leaf-batch N`（既定 8）、`--pipeline`: 評価中のバッチを別タスク（`Task.detached`）に置いたまま次のバッチを選択・準備する（`Search.runPipelined`）。バッチ k+1 はバッチ k の backup 前に選ぶので木は変わる（バッチ拡大と同種）。rollout value では使わない。予約の取り残し・世代無効化・締め切りのテスト 4 件（`PipelinedSearchTests`）。
- `--max-nodes N`（既定 100,000。9 路で 1 ノード約 30 KB、10 万 visits で RSS 3.1 GB）。
- 葉の局面（copy + play）とスナップショットはバッチ選択後にまとめて作る（`SearchNode.materialize`）。探索の葉では SHA-256 フィンガープリントを作らず、木の再利用時に必要な子だけ計算する（直列の準備時間 -30%）。既定設定で変更前後の 5 回の genmove（analyze 出力を含む）が完全一致することを確認。`concurrentPerform` による準備の並列化は、Core の盤コードの共有参照カウント（`swift_retain/release`）の取り合いで直列より遅かったため不採用。

visits/秒（M5、wide512、3 局面の中盤、固定 2〜3 万 visits、KataGo 停止中）:

| 設定 | visits/秒 |
|---|---|
| metal、leafBatch 8（従来の既定） | 1,133 |
| metal-packed、64 | 4,333 |
| cpu-packed（1 コア）、32 | 6,011 |
| cpu-packed-mt、128 | 12,759 |
| cpu-packed-mt、256 | 16,320 |
| **cpu-packed-mt、256、`--pipeline`** | **23,732** |

内訳（cpu-packed-mt 256、pipeline なし、3 万 visits）: 評価 60%（並列）、葉の準備 25%（直列、1 葉あたり copy 3.1 µs = ほぼ BoardHistory の複製、play 約 6.9 µs、スナップショット約 3.4 µs）、選択 7%。評価コアは 4 本を超えると頭打ち（直列部が律速）。次の候補は Core 側（`BoardHistory.copy` が新規生成してから上書きするため盤を約 14 枚複製している、`play` の超コウ更新）だが、RinGo からの無改変移植なので未着手。

**RinGo との組み合わせ（`katago-mlx/.tools/champion.bin.gz`、b20c256、fp16）。**

CPU と GPU の干渉（LGN wide512 を cpu-packed で N コア並列、RinGo は MLX）:

| LGN のコア数 | RinGo GPU 評価 b8 / b32（評価/秒） | RinGo 実探索（visits/秒） | LGN 合計（局面/秒） |
|---|---|---|---|
| 0 | 688 / 836 | 560 | — |
| 4 | 664 / 818（-4% / -2%） | 480（-14%） | 26,380 |
| 8 | 642 / 809（-7% / -3%） | 469（-16%） | 36,758 |
| 10 | 589 / 767（-14% / -8%） | — | 41,233 |

GPU 評価はほぼ落ちないが、RinGo の探索は木の処理を CPU で行うため約 15% 遅くなる。LGN は RinGo の visits の約 50 倍の局面を同時に評価できる。

品質（RinGo champion の生ネット 1 visit と 400 visits の探索を KataGo analysis engine で計算。A = IchiGo 自身の対局のベンチ局面 1,500、B = 学習に使っていない強い棋譜 300 局からの 1,500 局面）:

| | RinGo 生ネット | LGN wide512 | LGN C=2048 |
|---|---|---|---|
| 深い探索の最善手が上位 8 手に入る率（A / B） | 0.964 / 0.981 | 0.791 / 0.851 | 0.831 / 0.869 |
| 上位 16 手（A / B） | 0.998 / 0.999 | 0.939 / 0.968 | 0.948 / 0.979 |

RinGo 生ネットの誤り（最善手が深い探索と違う: A 44.5% / B 43.7%、勝率が 0.1 以上ずれる: A 19.3% / B 36.1%）の予測 AUC（対局単位 5 分割のロジスティック回帰）: 最善手の誤りは RinGo 自身の不確かさ（1 - 最大確率、エントロピー）だけで A 0.708 / B 0.741、LGN との食い違いを足しても A 0.707〜0.714 / B 0.737〜0.738。勝率の誤りは A 0.501 → 0.509〜0.523、B 0.588 → 0.588〜0.600。**LGN は RinGo の生ネットにない情報をほとんど持たず、「難所の検出」「ルートの篩い」の役割は見込みがない。** 残る組み合わせ案（CPU での局所の死活読み、プレイアウト）は未検証。

**高速化の強さ確認（`p4-local-wide512-200k` 同士、固定開局 × 色交換、事故 0）。** 速くしても強さは自動的には上がらなかった:

| 対局（A vs B） | 条件 | A の勝-分-敗 | 平均（95% CI） |
|---|---|---|---|
| cpu-packed-mt 256 + pipeline vs 従来既定（metal、8） | 30 秒切れ負け 40 局 | 19-0-21 | 0.475（0.30, 0.65） |
| cpu-packed-mt 256 + pipeline vs cpu-packed-mt 8 | 固定 3,000 visits 40 局 | 3-0-37 | 0.075（0.00, 0.15） |

1 本目で A は 1 手あたり中央値 17,497 visits、B は 3,163 visits（5.5 倍読んで互角）。2 本目のとおり、評価待ちの葉（最大 512 = 256 × 2）が仮想損失（-1）で探索を浅く広く散らすため、1 visit あたりの質が大きく落ちていた。`--batch-growth D`（バッチ = ルート visits / D、8 から `--leaf-batch` まで）を追加し、固定 3,000 visits でバッチ 8 と 16 局ずつふるい分けた:

| 設定 | 対 バッチ 8（3,000 visits） | visits/秒（2 万 visits） |
|---|---|---|
| バッチ 8（基準） | — | 3,541 |
| バッチ 32 + pipeline | — | 7,316 |
| バッチ 64 + pipeline | 8-1-7（0.53） | 13,293 |
| growth 32、上限 256 + pipeline | 5-0-11（0.31） | 18,993 |
| growth 64、上限 256 + pipeline | 7-0-9（0.44） | 14,399 |

質を落とさなかったバッチ 64 + pipeline で持ち時間をそろえた: **対 従来既定（30 秒切れ負け 40 局）22-3-15（0.59、CI [0.46, 0.71]）**、1 手あたり中央値 10,762 visits 対 3,145 visits。勝ち越しだが 40 局では有意でない（+60 Elo 程度）。3.4 倍読んでもこの程度しか伸びないので、今の LGN は読みの量より知識（死活判定）で頭打ちになっている。CGOS に出すなら `--backend cpu-packed-mt --leaf-batch 64 --pipeline` が現時点の候補（GPU も空く）。

**phase 11 small（head v3、10 万 step、2026-09-23 夕方）。**

| モデル | hard top1 | expected MAE（検証） | 死石の検出率 | 死石群を丸ごと見逃した局面 | value MAE（ベンチ） |
|---|---|---|---|---|---|
| v1（x3data） | 0.367 | 0.270 | 0.290 | 922 / 3,382 | 0.264 |
| + 自分の対局の局面 | 0.366 | 0.266* | 0.330 | 821 | 0.237 |
| v2 | **0.373** | **0.258** | 0.373 | 730 | 0.238 |
| **v2 + 自分の対局の局面** | 0.367 | 0.260* | **0.456** | **673** | **0.220** |

\* 検証データに自分の対局が混ざるため v1/v2 の列とは比べられない（v1 と v2 は同じ検証局面）。生き石の正解率は 0.985〜0.991 で下がっていない。2 つの施策は足し算で効き、死石の検出率は 0.29 → 0.46。対局: v2 + 自分の対局 vs v1、各 800 visits、40 局で **29-3-8（0.76、CI [0.66, 0.86]）**。wide512 版 3 本は学習中。

**phase 11 wide512（head v3、20 万 step、2026-09-23 夜）。** v1 基準は既存の `p8-wide512-headv3-x3data-200k`（同じ検証局面で hard top1 0.385 / expected MAE 0.259）。

| モデル | hard top1 | expected MAE（検証） | 死石の検出率 | 丸ごと見逃した局面 | value MAE（ベンチ） |
|---|---|---|---|---|---|
| v1 基準（p8） | 0.385 | 0.259 | 0.346 | 772 / 3,382 | 0.252 |
| + 自分の対局の局面 | 0.372 | 0.262* | 0.361 | 735 | 0.224 |
| v2 | **0.403** | **0.238** | 0.440 | 665 | 0.211 |
| **v2 + 自分の対局の局面** | 0.386 | 0.231* | **0.462** | **588** | **0.191** |
| （参考）C=2048 v1（p10） | 0.412 | 0.259 | 0.403 | 593 | 0.263 |

\* 検証データに自分の対局が混ざるため v1/v2 の列とは比べられない。wide512 でも v2 と自分の対局の教師化は足し算で効き、死活ベンチでは C=2048 も上回った。`models/p11-wide512-*.ichigo` に export 済み（チャンピオンとの対局は未実施）。

**19 路の準備（UEC 杯）。** katagoarchive.org の kata1 学習棋譜（2026-09-18/19 の 2 日分、計約 33 万局、うち 19 路・置石なし 181,891 局。コミ 4.5〜8、ルールは地・面積が混在、約 4 割は途中局面からの開始）から 3 万局を選び、`ichigo features --sample-per-game`（新設。1 局から N 局面を gameId から決まる乱数で選ぶ）で 1 局 15 局面を v2 特徴で抽出中。サーバーからは既定の Python User-Agent だと 403 になるため、ブラウザ形式のヘッダーで取得した。教師は kata1-b18c384nbt（サーバーに配置済み、sha256 一致）。ラベルのルールは未決定（日本ルールにするなら teacher・dataset・manifest・engine の rulesId 拡張が要る）。

**新モデル vs チャンピオン（2026-09-23 夜）。** `p11-wide512-headv3-x3data-dagger-v2-200k` vs `p4-local-wide512-200k`、両者 `--backend cpu-packed-mt --leaf-batch 64 --pipeline`、30 秒切れ負け 40 局: **23-3-14（0.61、CI [0.46, 0.75]）**、事故 0。ただし 1 手あたり中央値は 1,389 visits 対 8,649 visits で、新モデルは 6 分の 1 しか読めていなかった。同じ設定の探索速度は v1 モデル（head v2/v3 とも）11,600〜12,600 visits/秒に対し v2 モデル 5,264 visits/秒。backend 単体は v1 7,462 / v2 6,883 局面/秒（B=64）でほぼ同じなので、原因は evaluator の中で 1 スレッドで行う v2 特徴の計算（`GroupFeatures.compute`: 石の配置から盤を組み直し、シチョウ探索、Benson、領域の塗り分け。1 局面あたり約 100 µs）。読みが 6 分の 1 でも勝ち越しているので、ここを速くすれば伸びる余地がある。19 路では 1 局面が 4 倍以上大きくシチョウも長いため、なおさら要対処。

### 2026-09-24: 19 路の教師データ（日本ルール）と学習開始

- 教師アダプタに `ichigo_train label --rules area|japanese` を追加（既定 area）。局面の再生・合法性はこれまでどおり `cgos-area-psk-v1`（超コウで合法な手順は単純コウでも合法）で、教師の採点だけ KataGo の日本ルール（単純コウ、地計算、セキの扱いあり、自殺手なし）にする。ラベルに `labelRules`、データセット manifest に `labelRules` を記録し、混在は build-data で拒否。
- 不具合修正: 局面行の初期配置（`initialStones`）は SGF 座標（`aa`）だが、KataGo の analysis engine には GTP 座標が要る。9 路の棋譜には初期配置がなく表に出ていなかったが、KataGo 学習棋譜の約 4 割は途中局面から始まるため、そのまま全部拒否されていた（試験 200 局面中 120）。`teacher.build_query` で GTP へ変換。
- ラベル: kata1-b18c384nbt、日本ルール、128 visits、GPU 7 本で 446,009 / 446,019 局面（拒否 10、1 本あたり約 12 局面/秒、約 1.5 時間）。コミは元の対局のもの（4.5〜8）をそのまま使い、コミを入力として学ばせる（本番は 6.5 を入れる）。
- データセット `dataset-19-v2-jp`: 学習 401,956 / 検証 21,680 / テスト 22,331、featureVersion 2、labelRules japanese。
- phase 19（`configs/experiments/phase19/`、GPU 0〜5）: small 局所 / 広域（`[1,1,2,1,4,1,8,1]`）各 10 万 step（0.3 秒/step）、wide512 局所 / 広域 / 中間（`[1,1,2,1,2,1,4,1]`）各 20 万 step（0.56 秒/step）、C=1024 広域 10 万 step（C=1024 は microBatch 128 で GPU メモリ不足のため 64 × 2 回累積）。検証は 4,000 局面に絞る。9 路では局所配線が良かったが、19 路では 8 層×3×3 の局所配線は半径 8 までしか届かないので範囲を比較する。

### 2026-09-25: v2 特徴と盤の高速化、集約層の試作（phase 20）

**v2 特徴の計算を速くした（出力は完全一致）。** 探索中のプロファイル（`sample`）で、v2 の群特徴は evaluator の中で 1 局面ずつ直列に計算され、その内訳は盤の組み立て 46%（石を 1 個置くたびに全連を作り直していた）、2 呼吸点の連のシチョウ探索 36%、Benson 11% だった。変更:
- `GroupFeatures.compute(layout:)`: 石をまとめて置き、連の作り直しは 1 回（`setStonesTolerant`）。合法な局面では途中で取り・自殺が起きないので、1 個ずつ置いた結果と同じ。
- `GroupFeatures.compute(batch:)`: バッチ内の局面を `concurrentPerform` で並列に計算（各局面が自分の `Board` を作る）。葉の準備の並列化では共有参照カウントの取り合いで遅くなったが、こちらは起きなかった。
- `Board`（IchiGoCore）: 着手と連の作り直しから `Set` の確保をなくした（訪問印の配列＋世代番号、盤上の点の一覧を 1 回だけ作る）。連の先頭・連の中の順序は変えていない（シチョウ探索の手順がこれに依存するため、差分更新にはしなかった）。
- 検証: 旧実装で作った盤の全状態（色・連の先頭・次の石・呼吸点数・コウ・ハッシュ・取った石の数）と v2 特徴のダイジェストを 9 路 12 局・19 路 3 局の乱数対局で固定し（`BoardGoldenTests`）、書き換え後も一致。石を 1 個ずつ置く旧方式との特徴の一致も乱数局面 500 以上で確認（`GroupFeatureTests`）。19 路の棋譜 60 局の `ichigo features` 出力は v1・v2 ともバイト単位で同一。Swift テスト 238 件通過。

| | 変更前 | 変更後 |
|---|---|---|
| 9 路探索、v2 モデル（wide512、cpu-packed-mt、64 + pipeline） | 4,700 visits/秒 | **11,200** |
| 9 路探索、v1 モデル（同） | 11,300 | 12,800 |
| 19 路の v2 群特徴（1 スレッド、CPU 時間） | 1.96 ms/局面 | 0.70 ms/局面 |

**集約層の試作（`Training/ichigo_train/aggregation.py`、設定キー `aggregation`）。** 2 入力ゲートが多数の入力をまとめられないことが律速か、を確かめるための実験用の層。ゲート層の出力の先頭チャンネルを置き換えるので、配線・head・チャンネル数は変えない。`.ichigo` には未対応（export は拒否）。
- `component-or`: 同じ状態の 4 連結成分（石の連、または空点の領域）の OR。`scope` chains（石の連だけ）/ chains+eyes（8 点以下の空領域も）/ all、`relaxation` prob（1−∏(1−x)）/ max。学習時の成分計算は最小ラベルの伝播（点数×直径）で、19 路でも点数×点数の行列を作らない。
- `threshold`: BNN 型のしきい値ノード（3×3 近傍から fanIn 本、学習した符号で反転、しきい値以上で 1）。`weights` tanh / ste（forward は符号、backward は tanh の勾配）、`outputSte`。
- 検査の道具: `Scripts/aggregation_check.py`（fixture、outputs = 死活ベンチ用の hard 出力、diag = 連の大きさ別の活性・勾配・飽和・hard 時の定数化率、しきい値ノードの soft/hard 一致）。Python の hard 出力は Swift の eval-batch と一致（ownership の差は最大 5e-5）。

別セッションの敵対レビュー（Codex）で指摘された点: (1) 確率的 OR は大きな成分で飽和し勾配が消える、(2) しきい値ノードは学習時の連続値と推論時の符号がずれる、(3) 推論コストの見積もりは実測が要る、(4)「2 入力が主因」はまだ言えない（幅・深さで伸びない結果は勾配・配線・離散化・最適化でも説明できる）、(5) 両案が失敗しても LGN の限界の証明ではない、(6) v3 特徴の効果予測は選択に使ったベンチとは別のデータで確かめる。長時間学習の前に以下を確認した。

初期状態の診断（検証 1,024 局面、演算は層 2・5 の後、32 チャンネル）:

| | 連の大きさ 8〜15 | 16 以上 |
|---|---|---|
| OR prob: 出力が 0.99 超の割合 | 0.97〜1.00 | 1.00 |
| OR prob: 入力への勾配（同じ点の非集約チャンネル比） | 約 1/1,000〜1/50 | 約 1/10⁷ |
| OR max: 飽和・勾配 | なし・同程度 | なし・同程度 |

しきい値ノードの soft/hard 一致（同じ hard 入力で）: tanh 84%、ste 100%（ずれは出力の sigmoid だけ）、ste + outputSte は完全一致。

推論コスト（C のマイクロベンチ、Apple M5 1 コア、9 路、32 局面を 1 語に詰めた形、1 局面・1 層あたり）: 連単位 OR 32ch はビット並列の塗り広げ 1.1 µs（平均 3.9 巡）、転置して連番号で集約 0.53 µs（出力一致）、しきい値ノード 64 個 × 入力 16 は加算回路で 2.9 µs（総当たりと一致）。small 全体は 78 µs（論理層 31、head 48）、wide512 は 147 µs。2 層入れても small の +2〜8% なので、比較相手は同じコストの small。

短い学習（small、12k step、seed 1、dataset-9x3-dagger-v2、GPU 0 に 7 本相乗り）:

| | hard top1 | expected MAE | 4k step の soft 損失 / soft と hard の差 | 死石の検出率 | 丸ごと見逃し | value MAE（ベンチ） |
|---|---|---|---|---|---|---|
| 基準 | 0.339 | 0.303 | 3.44 / 0.09 | 0.385 | 693 | 0.269 |
| OR chains prob | 0.341 | 0.304 | 3.48 / 0.23 | **0.455** | 624 | 0.272 |
| OR chains max | **0.347** | **0.298** | **3.41** / 0.11 | 0.440 | 700 | 0.267 |
| OR chains+eyes prob | 0.336 | 0.299 | 3.42 / 0.29 | 0.441 | 644 | 0.267 |
| しきい値 tanh | 0.294 | 0.301 | 3.51 / 1.63 | 0.364 | 655 | 0.272 |
| しきい値 ste | 0.325 | 0.301 | 3.45 / 0.60 | 0.408 | 655 | 0.270 |
| しきい値 ste + outputSte | 0.312 | 0.302 | 3.61 / 0.10 | 0.451 | **571** | **0.264** |

学習後の診断: hard の OR は 16 点以上の連で出力チャンネルの 7 割前後が定数（入力側は 2 割前後）。ゲート出力の半分近くが 1 なので大きな連の OR はほぼ常に 1 になり、効いているのは主に 2〜7 点の連と考えられる。しきい値ノードは学習後 |tanh w| が 0.81〜0.86 まで寄り、ste 系は soft/hard 一致 100%、tanh は 89〜92%。seed 1 本・短い学習なので差は目安。100k step・seed 2 本の比較（基準 / OR max / OR prob / しきい値 ste+outputSte、`configs/experiments/phase20/matrix-compare*.json`、基準の seed 0 は同一設定の `p11-small-headv3-x3data-dagger-v2-100k`）を GPU 0・1 で実行中。

**hard のまま過学習できるか（検証局面 512 を学習にも使う fixture、4k step、最後は全層 hard）。**

| | 損失 | top1 | expected MAE |
|---|---|---|---|
| 基準 | 2.733 | 0.449 | 0.070 |
| OR chains max | **2.606** | **0.477** | **0.060** |
| OR chains prob | 2.827 | 0.420 | 0.091 |
| OR chains+eyes prob | 2.757 | 0.447 | 0.103 |
| しきい値 ste + outputSte | 2.830 | 0.393 | 0.079 |
| しきい値 ste | 2.857 | 0.377 | 0.088 |
| しきい値 tanh | 2.990 | 0.352 | 0.095 |

OR max は hard でも基準よりよく当てはまる。しきい値ノードはどの方式も基準より当てはまりが悪く、学習の仕方に問題が残る（今回の比較にはいちばんましな ste + outputSte だけ残した）。

**v3 特徴（featureVersion 3、仕様 01 §1）。** 事前登録（プローブで選んだ 4 特徴・判定基準を、確認用データを作る前に固定）どおり、選択に使っていない新しい対局 220 局（`p11-wide512-headv3-x3data-dagger-v2-200k` 対 `p4-local-wide512-200k` 各 800 visits 160 局、`p11-small-...-dagger-v2-100k` 対 gnugo L10 60 局。開局は既存と重ならない乱数 4 手 80 通り）から 1 局 12 局面、計 2,640 局面を Mac の教師でラベル（評価専用）して確かめた。

| 死石の判別（石 96,115、うち死石 11,746） | AUC | 死石の検出率 |
|---|---|---|
| 基準（v2 最良モデルの ownership + 入力 32ch） | 0.933 | 0.562 |
| + v3 の 4 平面 | 0.954 | 0.614 |

伸びは +0.020（対局単位のブートストラップ 95% CI [+0.015, +0.026]）で、選択に使ったベンチでの +0.034 より小さい（選択の楽観分）。同じ対局で新モデル対チャンピオンは 115 勝 45 敗（0.72）、small 対 gnugo L10 は 26 勝 34 敗（0.43）。外した 4 チャンネル（2 手前の石、空点、盤端）は、同じプローブで死石の情報がほぼない（AUC 0.9334 → 0.9333）。
- 実装: `GroupFeaturesV3`（盤クラスを使わない配列、取りの読みは深さ 10・600 ノード）、`FeatureEncoder` v3、`ichigo reencode`（局面行の手順を再生して別の版で符号化し直す。positionId が再現しない行はエラー）。v2 → v2 の再符号化は 300 行すべてが元と同一。Python のプロトタイプとは 3 平面が完全一致、取りの読みは 99.4% 一致で、ずれはプロトタイプ側の不具合（守り手・攻め手の手を返り値で「成功」と表しており、点番号 0 の手が偽として扱われていた）。Swift 版の値で判定し直しても +0.020（CI [+0.015, +0.026]）。
- 追加の計算: 9 路で約 70 µs、19 路で約 0.28 ms（1 局面、1 スレッド）。
- データ: 学習データ 399,765 局面（positions-9 + batch2 + 自分の対局）を `reencode` で v3 にし、v2 との差が 4 チャンネルだけであることを確認。`dataset-9x3-dagger-v3` は v2 版と局面・ラベル・分割が同一（学習 315,173 / 検証 18,589 / テスト 17,168）。
- phase 21（`configs/experiments/phase21/`、GPU 6）: small v3 100k × seed 2 本。v2 の同じ構成（seed 0 = `p11-small-...-dagger-v2-100k`、seed 1 = `p20-small-base-s1-100k`）と比べる。
